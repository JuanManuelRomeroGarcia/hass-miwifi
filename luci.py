...
"""Luci API Client (revised).
- Robust handling of httpx.AsyncClient closed-state with single retry + safe recreation
- Single-client lifecycle friendly (no per-refresh creation)
- Optional cleanup when recreating client to avoid resource leaks
"""
...
class LuciClient:
    ...
    def __init__(
        ...
        :param client: AsyncClient: AsyncClient object (should live for integration lifetime)
    ...
    async def _request_with_retry(self, method: str, url: str, **kwargs) -> Response:
        """
        Execute GET/POST on self._client. If the client is closed, recreate it once and retry.
        This prevents RuntimeError('client has been closed') crashes and avoids memory leaks
        by keeping a single persistent client instance.
        """
        try:
            ...
            if "client has been closed" in str(e).lower():
                # Try to close defensively (if it was half-open) then recreate.
                try:
    ...
    def _get_url(self, protocol: str | None = None) -> str:
    ...
    async def login(self) -> dict:
        ...
        try:
            ...
            response: Response = await self._request_with_retry(
                "post", _url, data=_request_data, timeout=self._timeout
            )
    ...
    async def get(
        ...
        :param use_stok: bool: whether to include stok
    ...
    async def misystem_info(self) -> dict:
        ...
        try:
            ...
            self._debug("misystem_info status() failed", self._url or "", e, "misystem_info")
        try:
            return await self.get("misystem/")
        except Exception as e:
            self._debug("misystem_info misystem/ failed", self._url or "", e, "misystem_info")
        return {}
    ...
    def _debug(self, message: str, url: str, content: Any, path: str, is_only_log: bool = False) -> None:
