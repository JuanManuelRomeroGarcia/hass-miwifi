...
class LuciUpdater(DataUpdateCoordinator):
    ...
    def __init__(
        ...
        # Cache y backoff para macfilter_info para evitar machacar el router
        self._macfilter_cache: dict[str, Any] | None = None
        self._macfilter_cache_ts: datetime | None = None
        self._macfilter_backoff_until: datetime | None = None
    ...
    async def _async_get_macfilter_info(self) -> dict:
        """
        Obtener macfilter_info con cache + backoff.

        - Si estamos en periodo de backoff → no llamamos al router, devolvemos {}.
        - Si tenemos cache reciente → devolvemos cache.
        - Si falla (timeout/otro error) → aplicamos backoff y devolvemos {}.
        """
        now = datetime.now().replace(microsecond=0)

        # 1) Si estamos en backoff, devolvemos cache (o {} si no hay)
        if self._macfilter_backoff_until and now < self._macfilter_backoff_until:
            return self._macfilter_cache or {}

        # 2) Si cache reciente, reutilizar
        cache_ttl = max(self._scan_interval, 30)  # al menos 30s
        if self._macfilter_cache_ts:
            age = (now - self._macfilter_cache_ts).total_seconds()
            if age < cache_ttl:
                return self._macfilter_cache or {}

        # 3) Llamada real al router
        macfilter_info: dict = {}
        try:
            macfilter_info = await asyncio.wait_for(
                self.luci.macfilter_info(), timeout=4
            )
            # éxito → cache y sin backoff
            self._macfilter_cache = macfilter_info or {}
            self._macfilter_cache_ts = now
            self._macfilter_backoff_until = None
            return self._macfilter_cache
        ...
        # 4) Fallo → activar backoff
        backoff_secs = max(self._scan_interval * 4, 60)  # mínimo 1 min o 4 ciclos
        self._macfilter_backoff_until = now + timedelta(seconds=backoff_secs)
        self._macfilter_cache = {}
        self._macfilter_cache_ts = now
        return {}
    ...
    async def _async_prepare_init(self, data: dict) -> None:
        ...
        :param data: dict
        """
        # Si ya tenemos datos básicos y no es el primer update, no repetimos
        if (
            not self._is_first_update
        ...
        # Modelo y fabricante de cara a HA
        ...
        # Mapear el hardware a la enum Model, pero NUNCA abortar si no se reconoce
        hw_value = response.get("hardware")
        if hw_value:
            try:
                data[ATTR_MODEL] = Model(str(hw_value).lower())
            except Exception:
                # Si no se reconoce el modelo, usamos NOT_KNOWN pero no rompemos la integración
                data[ATTR_MODEL] = Model.NOT_KNOWN
                await self.hass.async_add_executor_job(
                    _LOGGER.warning,
                    "[MiWiFi] Hardware '%s' no identificado, usando Model.NOT_KNOWN",
                    hw_value,
                )

        # Importante: NO lanzamos LuciError aquí ni cambiamos self.code.
        # El router se considera soportado aunque el modelo no esté en la enum.
    ...
    async def _async_prepare_status(self, data: dict) -> None:
        ...
        if "mem" in response and isinstance(response["mem"], dict):
            ...
            if "total" in response["mem"]:
                data[ATTR_SENSOR_MEMORY_TOTAL] = parse_memory_to_mb(
                    response["mem"]["total"]
                )
        ...
        with contextlib.suppress(LuciError):
            ...
            if "uptime" in response:
                data = {
                    ...
                    ATTR_BINARY_SENSOR_VPN_STATE: int(
                        float(response["uptime"])
                    ) > 0,
                }
    ...
    async def _async_prepare_wan(self, data: dict) -> None:
        ...
        try:
            ...
            if not isinstance(info, dict):
                await self.hass.async_add_executor_job(
                    _LOGGER.debug,
                    "WAN info['info'] is not a dict, got: %s",
                    type(info),
                )
                info = {}
            elif not info:
            ...
            if (
                isinstance(ipv4_list, list)
                and ipv4_list
                and isinstance(ipv4_list[0], dict)
            ):
    ...
    async def _async_prepare_devices(self, data: dict) -> None:
        ...
        macfilter_info: dict = await self._async_get_macfilter_info()
    ...
    async def _async_prepare_device_list(self, data: dict) -> None:
        ...
        mac_to_ip: dict[str, str] = {
            ...
            if "ip" in device
            and len(device["ip"]) > 0
            and ATTR_TRACKER_MAC in device
        }
        add_to: dict = {}
        ...
        for device in response["list"]:
            ...
            if (
                ...
                if (
                    ...
                    and device[ATTR_TRACKER_MAC]
                    not in integration[UPDATER].devices
            ...
            # 🔢 Aquí aplicamos el total de tráfico si lo tenemos
        ...
        for mac, device in devices.items():
            ...
            if mac not in self._moved_devices:
                ...
                async_dispatcher_send(
                    self.hass,
                    SIGNAL_NEW_DEVICE,
                    device | {ATTR_TRACKER_IS_RESTORED: True},
                )
    ...
    async def add_device(
        ...
        if (
            ATTR_TRACKER_FIRST_SEEN
            not in self.devices[device[ATTR_TRACKER_MAC]]
        ):
        ...
        if "new_status" not in self.data:
            ...
            code: str = (connection or Connection.LAN).name.replace(
                "WIFI_", ""
            )
    ...
    def _build_device(
        ...
        ip_attr: dict | None = (
            device["ip"][0] if "ip" in device and device["ip"] else None
        )

        # Ajuste para forzar tipos en modo force_load
        ...
        # Obtener entry_id de forma robusta: si no viene en el device, usar el del updater
        entry_id = device.get(ATTR_TRACKER_ENTRY_ID, self._entry_id)
        updater_entry_id = device.get(ATTR_TRACKER_UPDATER_ENTRY_ID, entry_id)

        return {
            ATTR_TRACKER_ENTRY_ID: entry_id,
            ATTR_TRACKER_UPDATER_ENTRY_ID: updater_entry_id,
            ...
            if ip_attr and "downspeed" in ip_attr
            else 0.0,
            ATTR_TRACKER_UP_SPEED: float(ip_attr["upspeed"])
            if ip_attr and "upspeed" in ip_attr
            else 0.0,
    ...
    async def _async_prepare_topo(self) -> None:
        ...
        try:
            ...
            if (
                not topo_data
                or not isinstance(topo_data, dict)
                or "graph" not in topo_data
            ):
                await self.hass.async_add_executor_job(
                    _LOGGER.info,
                    "[MiWiFi] No topology graph data available for router at %s",
                    self.ip,
                )
            ...
            if not isinstance(graph, dict):
                await self.hass.async_add_executor_job(
                    _LOGGER.error,
                    "[MiWiFi] ❌ Invalid topology graph format (not dict): %s",
                    graph,
                )
                self.data["topo_graph"] = None
                return
            ...
            for entity in self.hass.states.async_all("sensor"):
                ...
                if eid.startswith("sensor.topologia_miwifi") or eid.startswith(
                    "sensor.miwifi_topology"
                ):
                    ...
                    if mac_entity and mac_entity == mac_graph:
                        clean_attributes = {
                            ...
                            "icon": entity.attributes.get(
                                "icon", "mdi:network"
                            ),
                            "friendly_name": entity.attributes.get(
                                "friendly_name", "Topología MiWiFi"
                            ),
                        }
                        self.hass.states.async_set(
                            eid, entity.state, clean_attributes
                        )
            try:
                ...
                for node in nodes:
                    if isinstance(node, dict):
                        ...
                        if node_ip and node_ip != self.ip:
                            if (
                                node_ip
                                not in self.hass.data.get(DOMAIN, {})
                            ):
                                await self.hass.async_add_executor_job(
                                    _LOGGER.warning,
            ...
            await self.hass.async_add_executor_job(
                _LOGGER.warning,
                "[MiWiFi] Failed to get topology graph for router at %s: %s",
                self.ip,
                e,
            )
            self.data["topo_graph"] = None
            ...
            await self.hass.async_add_executor_job(
                _LOGGER.error,
                "[MiWiFi] Unexpected error while getting topology graph for router at %s: %s",
                self.ip,
                e,
            )
            self.data["topo_graph"] = None
    ...
    async def _async_prepare_compatibility(self) -> None:
        ...
        try:
            ...
            router_model = self.data.get(
                "model", self.data.get(ATTR_MODEL, "unknown")
            )
            await self.hass.async_add_executor_job(
                _LOGGER.info,
    ...
    async def _async_prepare_nat_rules(self) -> None:
        ...
        try:
            ...
            self.data["nat_rules"] = {
                ...
                "total": len(data1.get("list", []))
                + len(data2.get("list", [])),
            }
            await self.hass.async_add_executor_job(
                _LOGGER.debug,
                "[MiWiFi] NAT rules loaded for sensor: %s",
                self.data["nat_rules"],
            )
        except Exception as e:
            await self.hass.async_add_executor_job(
                _LOGGER.warning,
                "[MiWiFi] Error while retrieving NAT rules for sensor: %s",
                e,
            )
            self.data["nat_rules"] = {
                "ftype_1": [],
    ...
    async def _async_notify_new_device(self, name: str, mac: str) -> None:
        ...
        title = notify_trans.get(
            "new_device_title", "New Device Detected on MiWiFi"
        )
        ...
        message = (
            message_template.replace("{name}", name).replace("{mac}", mac)
        )
...
def async_get_updater(hass: HomeAssistant, identifier: str) -> LuciUpdater:
    ...
    if (
        identifier in hass.data[DOMAIN]
        and UPDATER in hass.data[DOMAIN][identifier]
    ):
...
async def async_update_panel_entity(
    hass: HomeAssistant, updater: LuciUpdater, async_add_entities=None
):
    ...
    entity_id = (
        f"update.miwifi_{mac.replace(':','_')}_miwifi_panel_frontend"
    )
    ...
    if is_main:
        ...
        if not entry and async_add_entities:
            await hass.async_add_executor_job(
                _LOGGER.debug,
                "[MiWiFi] 🟢 Creating update panel (%s main selection)",
                source,
            )
            panel_entity = MiWifiPanelUpdate(
                f"{updater.entry_id}_miwifi_panel", updater
            )
            async_add_entities([panel_entity])
        elif not entry:
            await hass.async_add_executor_job(
                _LOGGER.debug,
                "[MiWiFi] ⚠ Cannot create update panel (%s main) because async_add_entities is not available",
                source,
            )
    else:
        if entry:
            await hass.async_add_executor_job(
                _LOGGER.debug,
                "[MiWiFi] 🔴 Removing update panel because it is no longer main",
            )
